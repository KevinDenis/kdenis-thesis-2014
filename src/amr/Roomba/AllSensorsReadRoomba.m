function [Bump, Cliff, Dist, Angle,pCharge]=AllSensorsReadRoomba(serPort)
%[Bump, Cliff, Dist,pCharge]=AllSensorsReadRoomba(serPort)
%   Original :
...
%       * the encoder values are wrong (angle and distance) there for a
%         compatibility factor is added
...
warning off
global td odoAngleCor odoDistCor
sensorPacket = [];
...
Dist = fread(serPort, 1, 'int16')/1000*odoDistCor; % convert to Meters, signed, average dist wheels traveled since last time called...caps at +/-32
Angle = fread(serPort, 1, 'int16')*pi/180*odoAngleCor ; % convert to radians, signed,  since last time called, CCW positive
...
Cliff=CliffLft||CliffRgt||CliffFrntLft||CliffFrntRgt;
